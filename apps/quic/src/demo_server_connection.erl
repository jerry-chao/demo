...
new_conn(Conn, #{version := _Vsn}, #{stream_opts := SOpts} = S) ->
    case quicer_stream:start_link(demo_server_stream, Conn, SOpts) of
        {ok, Pid} ->
...
new_stream(Stream, Flags, #{ conn := Conn, streams := Streams
    ...
    %% Spawn new stream
    case quicer_stream:start_link(demo_server_stream, Stream, Conn, SOpts, Flags) of
        {ok, StreamOwner} ->

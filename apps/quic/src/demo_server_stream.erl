...
init_handoff(Stream, StreamOpts, Conn, #{flags := Flags}) ->
    ...
    ?LOG_DEBUG("init_handoff ~p", [{InitState, StreamOpts}]),
    {ok, InitState}.
...
send_complete(_Stream, true = _IsCanceled, S) ->
    ?LOG_DEBUG("~p : send is canceled", [?FUNCTION_NAME]),
    {ok, S}.


send_shutdown_complete(_Stream, _Flags, S) ->
    ?LOG_DEBUG("~p : stream send is complete", [?FUNCTION_NAME]),
    {ok, S}.
...
handle_stream_data(Stream, Bin, _Flags, #{is_unidir := false} = State) ->
    ...
    ?LOG_DEBUG(#{stream => Stream, data => Bin, module => ?MODULE, dir => bidir}),
    case Bin of
        <<CNo:32, SNo:32>> ->
            ?LOG_DEBUG(#{stream => Stream, data => {CNo, SNo}, module => ?MODULE, dir => bidir}),
            {ok, _} = quicer:send(Stream, <<"dataok">>);
        <<"ping">> ->
...
handle_stream_data(Stream, Bin, _Flags, #{is_unidir := true, peer_stream := PeerStream, conn := Conn} = State) ->
    ?LOG_DEBUG(#{stream => Stream, data => Bin, module => ?MODULE, dir => unidir}),
    ?LOG_DEBUG("Server recv: ~p from ~p", [Bin, Stream] ),
...
passive(_Stream, undefined, S)->
    ?LOG_DEBUG("Steam go into passive mode"),
    {ok, S}.
